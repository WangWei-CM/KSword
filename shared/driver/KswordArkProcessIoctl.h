...
#define KSWORD_ARK_IOCTL_FUNCTION_TERMINATE_PROCESS_THREADS 0x84FUL
...
// 结束目标进程的全部线程协议：
// - 输入：PID 和每个线程的退出状态；R0 自行解析 EPROCESS/ETHREAD；
// - 处理：仅执行线程扫描终止，不执行完整进程终止流水线的句柄终止或内存清零阶段；
// - 输出：无；完成状态通过 DeviceIoControl 成功与否以及驱动日志返回。
#define IOCTL_KSWORD_ARK_TERMINATE_PROCESS_THREADS \
    CTL_CODE( \
        KSWORD_ARK_IOCTL_DEVICE_TYPE, \
        KSWORD_ARK_IOCTL_FUNCTION_TERMINATE_PROCESS_THREADS, \
        METHOD_BUFFERED, \
        FILE_WRITE_ACCESS)

typedef struct _KSWORD_ARK_TERMINATE_PROCESS_THREADS_REQUEST
{
    unsigned long processId;
    long exitStatus;
} KSWORD_ARK_TERMINATE_PROCESS_THREADS_REQUEST;
