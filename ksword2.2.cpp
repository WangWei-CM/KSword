...
#define kswordversion "Ksword2.2"
...
int main(int argc, char* argv[])
{
	...
	NormalStart:
	    ...
	    for (int i = 0; i < waitTime; ++i) {
	        if ((GetAsyncKeyState(VK_CONTROL) & 0x8000) && !ctrlPressed) {
	            ctrlPressed = true;
	            pressCount++;
...
int show_logo()
{
cprint("====================Kali Sword V2.2 For Public=====================",87);
cout<<endl;
int osver=winver();
if(osver<=9){
...
void keyboardListener() {
    while (true) {
        if (GetAsyncKeyState(VK_CONTROL) & 0x8000) { // 检测Ctrl键是否被按下
            ctrlCount++;
            while (GetAsyncKeyState(VK_CONTROL) & 0x8000) {} // 等待Ctrl键释放
            cout << "Ctrl pressed. Count: " << ctrlCount << endl;

            if (ctrlCount >= 3) {
                hideWindow(); // 如果按下次数达到3次，则隐藏窗口
            }
        }

        if (GetAsyncKeyState(VK_SHIFT) & 0x8000) { // 检测Shift键是否被按下
            showWindow(); // 如果按下Shift键，则显示窗口
            ctrlCount = 0; // 重置计数器
            cout << "Window shown." << endl;
        }

        this_thread::sleep_for(milliseconds(100)); // 每100ms检测一次
    }
}
