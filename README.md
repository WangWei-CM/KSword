...
### 主要能力
- 进程分析与控制（枚举、详情、创建、终止、挂起/恢复、R0 可恢复隐藏等）
- 网络抓包与连接管理（过滤、连接管控、缓存诊断）
- 内存扫描与查看（区域、搜索、十六进制查看、书签/断点）
- 驱动与内核信息管理（服务操作、对象类型、NtQuery 结果）
- 注册表浏览与异步搜索（编辑、导入/导出）
- 日志输出与调用链追踪（GUID 级追踪）
- Taskbar 辅助程序提供顶部 AppBar、音频/CPU/网络状态展示，并支持 `S O S Enter` 键盘序列快速拉起主程序
...
| 进程 | `进程列表`、`创建进程`、进程详情窗口（`详细信息`/`线程`/`操作`/`模块`/`令牌`） | 进程枚举与树状/列表切换、右键动作（结束/挂起/恢复/关键进程/优先级/打开目录/R0 可恢复隐藏）、CreateProcess 参数化创建、线程与模块明细、令牌与注入相关调试动作。 |
...
| 设置 | `外观与启动` | 主题模式（跟随系统/浅色/深色）、背景图路径与透明度、默认启动页签、启动最大化、启动后默认最高级置顶、启动自动请求管理员权限。 |
...
- `Taskbar/`：Taskbar 相关工程，包含顶部 AppBar、状态展示和 SOS 快速拉起主程序功能
- `KswordHUD/`：HUD 相关工程
...
### Key Capabilities
- Process inspection and control (enumeration, details, creation, terminate, suspend/resume, reversible R0 hiding)
...
- Taskbar helper with a top AppBar, audio/CPU/network status display, and an `S O S Enter` keyboard sequence to launch the main app

### Repository Layout
- `Ksword5.1/`: Main solution and core source code
- `Taskbar/`: Taskbar-related helper project with AppBar status display and SOS quick launch
