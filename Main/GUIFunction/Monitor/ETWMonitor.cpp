...
void ETWEnumThreadProc(ETWEnumThreadTask task) {
    ...
    for (size_t i = task.startIdx; i < task.endIdx; ++i) {
        ...
        kItem.SetProcess(0, C("并行枚举ETW Provider..."), totalProgress);
    }
}
void EnumerateAllEtwProvidersWithEvents() {
    ...
    const int TOTAL_PROGRESS_ID = 0;                // 总进度条ID（ID=0）
...
void ETWMonitorMain() {
    if (ImGui::CollapsingHeader(C("ETW监控"))) {
        ...
        ImGui::BeginChild("scrolling", ImVec2(0, ImGui::GetContentRegionAvail().y * 0.5f - ImGui::GetStyle().ItemSpacing.y), true,
        ...
        if (ImGui::Button(C("添加自定义Provider") )&& g_etwMonitor.IsRunning()) {
            ...
            // 尝试从输入字符串解析GUID
            HRESULT hr = CLSIDFromString(wideGuid.data(), &customGuid);
            if (SUCCEEDED(hr)) {
                bool result = g_etwMonitor.AddProvider(customGuid, customLevel);
                if (result) {
                    kLog.info(C(C("成功添加自定义Provider: " + std::string(customGuidInput))), CURRENT_MODULE);
                    // 清空输入框
        ...
        ImGui::BeginChild("ETWEventTablescrolling", ImVec2(0, ImGui::GetContentRegionAvail().y * 0.5f - ImGui::GetStyle().ItemSpacing.y), true,
            ImGuiWindowFlags_HorizontalScrollbar);
