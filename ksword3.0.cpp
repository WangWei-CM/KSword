...
#define Kswordversion "Ksword3.0"
#define AdminPassword "Admin" 
#define kswordversiona 3
#define kswordversionb 0
...
//	cin.ignore(numeric_limits<std::streamsize>::max(), '\n');
	...
	if(directoryExists(constpath)){path=cmdpara;goto shellstart;}
	else{
		string tmppath=path;
		tmppath+=cmdpara;
    ...
    // 获取当前exe的完整路径
		...
		hack();
			...
			if(cmdpara=="update"){
				...
				pkgname=cmdpara;
				for(int i=1;i<=rcnumber;i++){
					cout<<aptinfo[i].filename<<aptinfo[i].year<<aptinfo[i].month<<aptinfo[i].day<<aptinfo[i].filesize<<endl;
					if(pkgname==aptinfo[i].filename){
						string installcmd;
						installcmd+="curl -o "+localadd+aptinfo[i].filename+" "+"http://"+serverip+":80/kswordrc/"+aptinfo[i].filename;
						cout<<installcmd<<endl;
						cout<<"Recources found."<<aptinfo[i].filesize<<"MB disk space will be used.Continue?(y/n)";
						char yon;
						cin>>yon;
			...
			cout<<"Can't find such recuorse."<<endl;
    		}
		}
