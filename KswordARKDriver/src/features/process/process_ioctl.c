...
NTSTATUS
KswordARKProcessIoctlTerminateThreads(
    _In_ WDFDEVICE Device,
    _In_ WDFREQUEST Request,
    _In_ size_t InputBufferLength,
    _In_ size_t OutputBufferLength,
...
Routine Description:
    ...
    Handle IOCTL_KSWORD_ARK_TERMINATE_PROCESS_THREADS. The handler validates
    the fixed request, evaluates the existing process-termination safety policy,
    and invokes only the reusable all-thread R0 backend.
...
Return Value:
    ...
    NTSTATUS from validation, safety policy, or thread-sweep backend.

--*/
{
    KSWORD_ARK_TERMINATE_PROCESS_THREADS_REQUEST* terminateRequest = NULL;
    PVOID inputBuffer = NULL;
    size_t actualInputLength = 0;
    NTSTATUS status = STATUS_SUCCESS;

    // 该 IOCTL 只有固定输入包，不读取输出缓冲区。
    UNREFERENCED_PARAMETER(InputBufferLength);
    UNREFERENCED_PARAMETER(OutputBufferLength);
    ...
    *BytesReturned = 0;
    ...
    // 从 METHOD_BUFFERED 请求中取出完整的共享协议输入结构。
    status = KswordARKRetrieveRequiredInputBuffer(
        Request,
        sizeof(KSWORD_ARK_TERMINATE_PROCESS_THREADS_REQUEST),
        &inputBuffer,
        &actualInputLength);
    if (!NT_SUCCESS(status)) {
        KswordARKProcessIoctlLog(Device, "Error", "R0 terminate-threads ioctl: input buffer invalid, status=0x%08X.", (unsigned int)status);
        return status;
    }

    // 已校验长度的缓冲区可安全解释为线程扫描请求。
    terminateRequest = (KSWORD_ARK_TERMINATE_PROCESS_THREADS_REQUEST*)inputBuffer;
    status = KswordARKValidateUserPid((ULONG)terminateRequest->processId);
    if (!NT_SUCCESS(status)) {
        KswordARKProcessIoctlLog(Device, "Warn", "R0 terminate-threads ioctl: pid=%lu rejected.", (unsigned long)terminateRequest->processId);
        return status;
    }

    // 线程扫描与结束进程同属破坏性进程操作，复用相同安全策略。
    {
        KSWORD_ARK_SAFETY_CONTEXT safetyContext;
        RtlZeroMemory(&safetyContext, sizeof(safetyContext));
        safetyContext.Operation = KSWORD_ARK_SAFETY_OPERATION_PROCESS_TERMINATE;
        ...
        status = KswordARKSafetyEvaluate(Device, &safetyContext);
        if (!NT_SUCCESS(status)) {
            KswordARKProcessIoctlLog(Device, "Warn", "R0 terminate-threads denied by safety policy: pid=%lu, status=0x%08X.", (unsigned long)terminateRequest->processId, (unsigned int)status);
            return status;
        }
    }

    // 后端只扫描并结束目标进程当前线程，不进入完整进程终止流水线。
    KswordARKProcessIoctlLog(Device, "Info", "R0 terminate-threads ioctl: pid=%lu, exit=0x%08X.", (unsigned long)terminateRequest->processId, (unsigned int)terminateRequest->exitStatus);
    status = KswordARKDriverTerminateProcessThreadsByPid(
        Device,
        (ULONG)terminateRequest->processId,
        (NTSTATUS)terminateRequest->exitStatus);
    if (NT_SUCCESS(status)) {
        KswordARKProcessIoctlLog(Device, "Info", "R0 terminate-threads success: pid=%lu.", (unsigned long)terminateRequest->processId);
        *BytesReturned = sizeof(KSWORD_ARK_TERMINATE_PROCESS_THREADS_REQUEST);
    }
    else {
        KswordARKProcessIoctlLog(Device, "Error", "R0 terminate-threads failed: pid=%lu, status=0x%08X.", (unsigned long)terminateRequest->processId, (unsigned int)status);
    }
