...
#define Kswordversion "Ksword3.1"
#define AdminPassword "Admin" 
#define kswordversiona 3
#define kswordversionb 1 

string serverip="159.75.66.16";
string localadd="c:\\kswordrc\\";
...
string getCmdResultdamn(const string&);//get cmd result.It's damn,dont use it. use run_cmd.
string run_cmd(const string&);
string usrname;
...
int readFile(const string& path) {
    ...
    file>>ws;
    if((kswordversion.a>kswordversiona)||((kswordversion.a==kswordversiona)&(kswordversion.b>kswordversionb))){
    	cout<<"upgrade error.Update this program first."<<endl;
    	return 2;
	}
    getline(file,poo);
    file>>rcnumber;
...
string run_cmd(const std::string& cmd) {
    char buffer[1024];
    std::string result;
    FILE* pipe;
    // 打开命令管道
    if ((pipe = _popen(cmd.c_str(), "r")) == NULL) {
        std::cerr << "_popen() failed" << std::endl;
        return ""; // 返回空字符串表示失败
    }
    // 读取命令执行过程中的输出
    while (fgets(buffer, sizeof(buffer), pipe) != NULL) {
        result += buffer; // 将读取到的字符串追加到结果中
    }
    // 关闭管道
    int status = _pclose(pipe);
    if (status == -1) {
        std::cerr << "_pclose() failed" << std::endl;
        return ""; // 返回空字符串表示失败
    }
    return result; // 返回命令执行的输出结果
}
int winver(){
	std::string vname;
	//先判断是否为win8.1或win10
...
int show_logo(){
cprint("====================Kali Sword V3.1 For Public=====================",87);
cout<<endl;
int osver=winver();
if(osver<=9){
...
string getCmdResultdamn(const string &strCmd){
    char buf[102400] = {0};
    FILE *pf = NULL;
...
//	cin.ignore(numeric_limits<std::streamsize>::max(), '\n');
	...
	if (nowcmd==cmd[1]){
		...
		else if (helpcmd=="apt"){cout<<"apt +:install <package name>/update/upgrade."<<endl;}
		else if (helpcmd=="setrcpath"){cout<<"use it to set apt's work dir'"<<endl;}
		else if (helpcmd=="c");
		...
		strre=getCmdResultdamn(strcmd[1]);
		cout << strre << endl;
		strre="";
	}
	...
	else if (nowcmd==cmd[12])
	{
		...
		cin>>tmp;
		strre=getCmdResultdamn(strcmd[tmp+3]);
		cout << strre << endl;
		strre="";
    }
	...
	else if (nowcmd==cmd[14])
	{
		string tmp;
		cprint("[ ! ]",6);
		cout<<"Same as command \"cd\".Enter default cmd path:";
		getline(cin,tmp);
		path=tmp;
		}
	...
	else if (nowcmd==cmd[22]){
		cin>>drivername;
		string driverpath;
		driverpath+=localadd;
		driverpath+=drivername;
		char* chardrivername = new char[drivername.length() + 1]; // +1 for the null terminator
		strcpy(chardrivername, drivername.c_str());
		char* chardriverpath = new char[driverpath.length() + 1]; // +1 for the null terminator
		strcpy(chardriverpath, driverpath.c_str());
		BOOL bRet = LoadNTDriver(chardrivername,chardriverpath);
    	if (!bRet)
    	{
	...
	else if (nowcmd==cmd[23]){
		char* chardrivername = new char[drivername.length() + 1]; // +1 for the null terminator
		strcpy(chardrivername, drivername.c_str());
    	UnloadNTDriver(chardrivername);
	}
	else if (nowcmd=="")
	{
		goto shellstart;
	...
	else if (nowcmd!=cmd[15])
	{
		...
		if(path!=""){
			if(path.length()<=3){
				if(path[0]=='c'){
				cmdcmd="cd\\ && "+nowcmd+' '+cmdpara;}
				else{
				cmdcmd=path[0];
				cmdcmd+=": && ";
				cmdcmd+=nowcmd+' '+cmdpara;
				}
				cout<<"Running command:"<<cmdcmd<<endl;
			}
			else{
				if(path[0]=='c'){
				cmdcmd="cd "+path+" && "+nowcmd+' '+cmdpara;}
				else{cmdcmd+=path[0];
				cmdcmd+=": && cd "+path;
				cmdcmd+=" && ";
				cmdcmd+=nowcmd+' '+cmdpara;
